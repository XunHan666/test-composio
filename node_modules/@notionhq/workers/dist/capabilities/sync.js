import { ExecutionError, RateLimitError } from "../error.js";
import { getPacerState, initPacerState } from "../pacer_internal.js";
import { resolveRuntimeInput } from "../runtime-input.js";
import { createCapabilityContext } from "./context.js";
import { writeError, writeOutput } from "./output.js";
import {
  parseSchedule
} from "./stateful-capability.js";
function createSyncCapability(key, syncConfiguration, pacerDeclarations) {
  const config = {
    databaseKey: syncConfiguration.database.key,
    primaryKeyProperty: syncConfiguration.database.config.primaryKeyProperty,
    mode: syncConfiguration.mode,
    schedule: syncConfiguration.schedule ? parseSchedule(syncConfiguration.schedule) : void 0
  };
  return {
    _tag: "sync",
    key,
    config,
    async handler(runtimeContext, options) {
      let executionResult;
      try {
        runtimeContext = await resolveRuntimeInput(runtimeContext);
        const capabilityContext = createCapabilityContext("sync");
        const state = runtimeContext?.state ?? runtimeContext?.userContext;
        initPacerState(runtimeContext?.pacers, pacerDeclarations);
        executionResult = await syncConfiguration.execute(state, capabilityContext);
      } catch (err) {
        if (!options?.concreteOutput) {
          writeError(err);
        }
        if (err instanceof RateLimitError) {
          if (!options?.concreteOutput) {
            const envelope = {
              _tag: "error",
              error: {
                _tag: "rate_limit",
                name: err.name,
                message: err.message,
                ...err.retryAfter !== void 0 ? { retryAfter: err.retryAfter } : {}
              }
            };
            writeOutput(envelope);
          }
          throw err;
        }
        const error = new ExecutionError(err);
        if (!options?.concreteOutput) {
          const envelope = {
            _tag: "error",
            error: {
              _tag: "generic",
              name: error.name,
              message: error.message
            }
          };
          writeOutput(envelope);
        }
        throw error;
      }
      const changes = executionResult.changes.map((change) => ({
        ...change,
        targetDatabaseKey: change.targetDatabaseKey ?? syncConfiguration.database.key
      }));
      const result = {
        changes,
        hasMore: executionResult.hasMore,
        nextUserContext: executionResult.nextState,
        nextPacerStates: getPacerState().pacers
      };
      if (options?.concreteOutput) {
        return result;
      } else {
        writeOutput({ _tag: "success", value: result });
      }
      return result;
    }
  };
}
function createXldbSyncCapability(key, syncConfiguration, pacerDeclarations) {
  return {
    _tag: "sync",
    key,
    config: {
      target: { type: "xldb", key: syncConfiguration.database.key },
      mode: syncConfiguration.mode,
      schedule: syncConfiguration.schedule ? parseSchedule(syncConfiguration.schedule) : void 0
    },
    async handler(runtimeContext, options) {
      let executionResult;
      try {
        runtimeContext = await resolveRuntimeInput(runtimeContext);
        const capabilityContext = createCapabilityContext("sync");
        const state = runtimeContext?.state ?? runtimeContext?.userContext;
        initPacerState(runtimeContext?.pacers, pacerDeclarations);
        executionResult = await syncConfiguration.execute(state, capabilityContext);
      } catch (err) {
        if (!options?.concreteOutput) {
          writeError(err);
        }
        if (err instanceof RateLimitError) {
          if (!options?.concreteOutput) {
            writeOutput({
              _tag: "error",
              error: {
                _tag: "rate_limit",
                name: err.name,
                message: err.message,
                ...err.retryAfter !== void 0 ? { retryAfter: err.retryAfter } : {}
              }
            });
          }
          throw err;
        }
        const error = new ExecutionError(err);
        if (!options?.concreteOutput) {
          writeOutput({
            _tag: "error",
            error: {
              _tag: "generic",
              name: error.name,
              message: error.message
            }
          });
        }
        throw error;
      }
      const result = {
        changes: executionResult.changes,
        hasMore: executionResult.hasMore,
        nextUserContext: executionResult.nextState,
        nextPacerStates: getPacerState().pacers
      };
      if (options?.concreteOutput) {
        return result;
      }
      writeOutput({ _tag: "success", value: result });
      return result;
    }
  };
}
export {
  createSyncCapability,
  createXldbSyncCapability
};

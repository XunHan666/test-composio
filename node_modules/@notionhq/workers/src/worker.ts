 ...
 */
export type ManagedDatabaseConfig<PK extends string, S extends Schema<PK>> = DatabaseConfigFields<
	PK,
	S
> & {
	type: "managed";
 ...
 */
export type AttachedDatabaseConfig<PK extends string, S extends Schema<PK>> = DatabaseConfigFields<
	PK,
	S
> & {
	type: "attached";
 ...
 */
export type DatabaseConfig<PK extends string, S extends Schema<PK>> =
	| ManagedDatabaseConfig<PK, S>
	| AttachedDatabaseConfig<PK, S>;
...
/** Internal type-erased alias for storing heterogeneous XLDB declarations. */
type RegisteredXldb = Pick<XldbHandle<XldbSchema, XldbUpsertKey<XldbSchema>>, "key" | "config">;
 ...
 */
export let registerXldb: (
	worker: Worker,
	xldb: Pick<XldbHandle<XldbSchema, XldbUpsertKey<XldbSchema>>, "key" | "config">,
) => void;
...
export class Worker {
	...
	#xldbs: Map<string, RegisteredXldb> = new Map();
	 ...
	 */
	sync<XS extends XldbSchema, UK extends XldbUpsertKey<XS>, Context = unknown>(
		key: string,
		config: XldbSyncConfiguration<XS, UK, Context>,
	): XldbSyncCapability;
	sync<
		PK extends string,
		S extends Schema<PK>,
		XS extends XldbSchema,
		UK extends XldbUpsertKey<XS>,
		Context = unknown,
	>(
		key: string,
		config: SyncConfiguration<PK, S, Context> | XldbSyncConfiguration<XS, UK, Context>,
	): SyncCapability | XldbSyncCapability {
		this.#validateUniqueKey(key);
		const capability = isXldbSyncConfiguration(config)
			? createXldbSyncCapability(key, config, Array.from(this.#pacers.values()))
			: createSyncCapability(key, config, Array.from(this.#pacers.values()));
	...
	customBlock(key: string, config: CustomBlockConfiguration): CustomBlockCapability {
		...
		if (
			...
			Array.from(this.#capabilities.values()).some(
				(existingCapability) =>
					existingCapability._tag === "custom_block" &&
					existingCapability.config.manifest.slashCommand === slashCommand,
			)
		) {
...
function isXldbSyncConfiguration<
	Context,
	PK extends string,
	S extends Schema<PK>,
	XS extends XldbSchema,
...
>(
	config: SyncConfiguration<PK, S, Context> | XldbSyncConfiguration<XS, UK, Context>,
): config is XldbSyncConfiguration<XS, UK, Context> {

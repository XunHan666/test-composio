...
/**
 * Throw this error from a workflow handler when retrying cannot make the
 * workflow succeed.
 *
 * The platform ends the workflow run immediately instead of scheduling
 * another attempt. Use this for permanent input or business-rule failures,
 * not for temporary network or service failures.
 * Pass an `ErrorOptions` object to preserve the original failure as this
 * error's `cause`.
 *
 * @example
 * ```ts
 * worker.workflow("processOrder", {
 *   // ...
 *   execute: async (event) => {
 *     if (!event.orderId) {
 *       throw new TerminalError("The event has no order ID");
 *     }
 *   },
 * });
 * ```

...
describe("Worker.tool", () => {
	...
	it("uses file-backed input instead of the argument input", async () => {
		...
		await capability.handler({ name: "Argument" });

		expect(execute).toHaveBeenCalledWith({ name: "File" }, expect.anything());
	});

	it("validates file-backed input against the tool schema", async () => {
		const capability = createToolCapability<{ name: string }, string>("sayHello", {
			title: "Say Hello",
			description: "Greet a user",
			schema: j.object({ name: j.string() }),
		...
		resolveRuntimeInputMock.mockResolvedValue({ name: 42 } as never);

		const result = await capability.handler({ name: "Argument" });

...
describe("Worker.sync", () => {
	...
	describe("handler output envelope", () => {
		it("uses file-backed runtime context instead of inline context", async () => {
			...
			const execute = vi.fn(async () => ({ changes: [], hasMore: false }));
			...
			await capability.handler({ state: { cursor: "inline" } }, { concreteOutput: true });

			expect(execute).toHaveBeenCalledWith({ cursor: "file" }, expect.anything());
		});
		...
		it("reports file-backed runtime context failures through the sync error envelope", async () => {
			...
			await expect(capability.handler()).rejects.toBeInstanceOf(ExecutionError);

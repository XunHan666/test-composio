...
export type XldbSyncHandlerResult<
	Schema extends XldbSchema,
	UpsertKey extends XldbUpsertKey<Schema>,
	State = unknown,
> = {
...
export type SyncConfiguration<
	S extends PropertySchema = PropertySchema,
	State = unknown,
> = CommonSyncConfiguration & {
	database: Database<S>;
	/**
...
export type SyncManifestConfig = {
	...
	target?: { _tag: "xldb"; name: string; config: unknown };
...
): Sync<SyncChange<S> & { targetDatabaseKey: string }>;
export function createSync<
	Schema extends XldbSchema,
	UpsertKey extends XldbUpsertKey<Schema>,
	State = unknown,
>(
	configuration: XldbSyncConfiguration<Schema, UpsertKey, State>,
): Sync<XldbSyncChange<Schema, UpsertKey>>;
export function createSync<
	S extends PropertySchema = PropertySchema,
	...
	State = unknown,
>(
	configuration: SyncConfiguration<S, State> | XldbSyncConfiguration<Schema, UpsertKey, State>,
): Sync {
	...
	return {
		...
		async handler(runtimeContext?: SyncRuntimeContext, options?: HandlerOptions) {
			let handlerResult:
				| SyncHandlerResult<S, State>
				| XldbSyncHandlerResult<Schema, UpsertKey, State>;
			try {
			...
			const changes =
				database._tag === "xldb"
					? handlerResult.changes
					: handlerResult.changes.map((change) => ({
							...change,
